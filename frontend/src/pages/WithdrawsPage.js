...
import { queryWithdraws } from "../apollo/gqlQuery"
...
const WithdrawsPage = (props) => {
  ...
  const { loading: loadingWithdraws, 
          ...
          subscribeToMore: subscribeToMoreWithdraws, 
          fetchMore: fetchMoreWithdraws,
          networkStatus: networkStatusWithdraws } = useQuery( queryWithdraws, { 
  ...
  useEffect(() => {
    if(!loadingWithdraws){
      if (dataWithdraws?.withdraws) {
        let { status, data: newData } = dataWithdraws?.withdraws
        if(status){
          setData(newData)
        }
      }
                            ...
                            () => [
                              {
                                ...
                                accessor: 'withdraw',
                                Cell: props =>{
                                    let withdraw = props.value
                                    return <div>{ withdraw?.balance }</div>
                                }
                              },
                              {
                                Header: 'User-Id',
